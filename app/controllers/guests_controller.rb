class GuestsController < ApplicationController
  ...
  def create
    ...
    if @guest.save
      ...
      RemindersMailer.remind_guest(@guest, @event).deliver_now
      # RemindersMailer.remind_guest(guest, event).deliver_later(wait_until: (event.time - 43200).seconds.from_now)
